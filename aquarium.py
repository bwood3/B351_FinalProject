...
class Aquarium:
    ...
    # - This means the fish vision is calculated as a square instead of a circle when used. -
    ...
    #return a copy of the grid with only what the fish can see and a list of all fish it can see
    def getVision(self, fish):
        ...
        for otherFish in self.fishes:
            distance = self.calc_noneuclidean_distance(fish.loc, otherFish.loc) #non-euclidean distance is used because the A* treats diagonal adjacencies equivalent to orthogonal adjacencies.
    ...
    # The update loop updates fish a number of times equal to their speed attribute, in descending order of speed.
    def updateSim(self):
        self.lifetime += 1
        maxSpeed = 0
    ...
    # Creates a food fish at a random location inside the grid
    def createFood(self):
